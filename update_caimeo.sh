...
# - Pull latest main
# - Prevent any dev frontend (npm start/react-scripts) from running
# - Build React frontend for nginx on :8080
# - Restart nginx, cloudflared, backend, and bot via systemd
# - Kill anything on :3000 and ensure only :8080/:8000 listen
...
info "Stopping and masking any caimeo-frontend.service..."
if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files | grep -q "^caimeo-frontend.service"; then
  ...
  info "caimeo-frontend.service not present; skipping mask."
fi

info "Killing any dev frontend processes (react-scripts/npm start/node:3000)..."
